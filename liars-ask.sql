...
--
-- Host: localhost
-- Generation Time: Nov 09, 2019 at 06:27 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AdvanceSearchAnnouncements` (IN `queryString` VARCHAR(50), IN `arrTags` VARCHAR(225), IN `dateFrom` TIMESTAMP, IN `dateTo` TIMESTAMP)  BEGIN
	SELECT announcements.id, announcements.title
        FROM announcements, tags_relationships
        WHERE announcements.id = tags_relationships.typeId
        AND announcements.title LIKE queryString
        AND announcements.created BETWEEN dateFrom AND dateTo
        AND tags_relationships.type = "announcements"
        AND tags_relationships.tagId IN (
            SELECT DISTINCT tags.id
            FROM announcements, tags_relationships, tags
            WHERE tags.id = tags_relationships.tagId
            AND announcements.id = tags_relationships.typeId
            AND tags_relationships.type = "announcements"
            AND tags.name IN (arrTags)
        );
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `AdvanceSearchBlogs` (IN `queryString` VARCHAR(50), IN `arrTags` VARCHAR(225), IN `dateFrom` TIMESTAMP, IN `dateTo` TIMESTAMP)  BEGIN
	SELECT blogs.id, blogs.title
        FROM blogs, tags_relationships
        WHERE blogs.id = tags_relationships.typeId
        AND blogs.title LIKE queryString
        AND blogs.created BETWEEN dateFrom AND dateTo
        AND tags_relationships.type = "blogs"
        AND tags_relationships.tagId IN (
            SELECT DISTINCT tags.id
            FROM blogs, tags_relationships, tags
            WHERE tags.id = tags_relationships.tagId
            AND blogs.id = tags_relationships.typeId
            AND tags_relationships.type = "blogs"
            AND tags.name IN (arrTags)
        );
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `AdvanceSearchQuestions` (IN `queryString` VARCHAR(50), IN `arrTags` VARCHAR(225), IN `dateFrom` TIMESTAMP, IN `dateTo` TIMESTAMP)  BEGIN
	SELECT questions.id, questions.title
        FROM questions, tags_relationships
        WHERE questions.id = tags_relationships.typeId
        AND questions.title LIKE queryString
        AND questions.created BETWEEN dateFrom AND dateTo
        AND tags_relationships.type = "questions"
        AND tags_relationships.tagId IN (
            SELECT DISTINCT tags.id
            FROM questions, tags_relationships, tags
            WHERE tags.id = tags_relationships.tagId
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `deleteAnnouncement` (`idAcm` INT)  BEGIN
	DELETE FROM tags_relationships WHERE typeId = idAcm;
   	DELETE FROM announcements WHERE id = idAcm;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `deleteBlog` (`idBlog` INT)  BEGIN
	DELETE FROM tags_relationships WHERE typeId = idBlog;
   	DELETE FROM blogs WHERE id = idBlog;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAnnouncements` ()  BEGIN
    ...
    AND type = "announcements";
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getBlogs` ()  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `requestRelatedAcmId` (IN `acmId` INT)  BEGIN
	SELECT * FROM announcements WHERE announcements.id = acmId;
	SELECT tags.name AS tagName, tags_relationships.id, tags_relationships.tagId
    FROM announcements, tags, tags_relationships
    WHERE announcements.id = acmId
    AND announcements.id = tags_relationships.typeId
    AND type = "announcements"
    AND tags_relationships.tagId = tags.id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `requestRelatedBlogId` (IN `blogId` INT)  BEGIN
	SELECT * FROM blogs WHERE blogs.id = blogId;
	SELECT tags.name AS tagName, tags_relationships.id, tags_relationships.tagId
    FROM blogs, tags, tags_relationships
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateAcm` (IN `title` VARCHAR(225), IN `body` TEXT, IN `isImportant` TINYINT(1), IN `acmId` INT)  BEGIN
	IF body != "" THEN
    	UPDATE announcements 
        SET title = title, isImportant = isImportant, body = body
        WHERE id = acmId;
    ELSE
        UPDATE announcements 
        SET title = title, isImportant = isImportant
        WHERE id = acmId;
    END IF;
END$$
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `AddABlogAfterAddTags` (`title` VARCHAR(225), `body` TEXT, `owner` INT) RETURNS INT(11) BEGIN
	INSERT INTO blogs (title, body, owner) VALUES (title, body, owner);
    RETURN LAST_INSERT_ID();
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `AddAnAnnouncementAfterAddTags` (`title` VARCHAR(225), `body` TEXT, `isImportant` TINYINT(1), `owner` INT) RETURNS INT(11) BEGIN
	DECLARE insertedId INT DEFAULT 0;
	INSERT INTO announcements (title, body, isImportant, owner) VALUES (title, body, isImportant, owner);
    SET insertedId = LAST_INSERT_ID();
    RETURN insertedId;
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `AddAnAnswer` (`body` TEXT, `owner` INT, `quesId` INT) RETURNS INT(11) BEGIN
	DECLARE insertedId INT DEFAULT 0;
	INSERT INTO answers (body, owner, quesId) VALUES(body, owner, quesId);
    SET insertedId = LAST_INSERT_ID();
    RETURN insertedId;
END$$
...
(58, 0, 'Clean-Code-Ruby', NULL),
(59, 0, 'CleanCode', NULL),
(60, 0, 'Ruby', NULL),
(61, 0, 'Rails', NULL),
(62, 0, 'web', NULL),
(63, 0, 'json-web-token', NULL),
(64, 0, 'js', NULL),
(65, 0, 'tress', NULL),
(66, 0, 'test', NULL),
(67, 0, 'final', NULL),
(68, 0, 'tags', NULL),
(69, 0, 'annoucement', NULL),
(70, 0, 'cfs', NULL),
(71, 0, 'edited', NULL),
(72, 0, 'olympic', NULL),
(73, 0, 'dream', NULL),
...
(51, 'blogs', 62, 3),
(50, 'blogs', 63, 3),
(74, 'announcements', 74, 3),
(75, 'announcements', 75, 3),
(23, 'questions', 1, 4),
(24, 'questions', 2, 4),
(25, 'questions', 55, 4),
(57, 'blogs', 62, 4),
(55, 'blogs', 63, 4),
(56, 'blogs', 64, 4),
(72, 'announcements', 37, 6),
(3, 'questions', 41, 6),
(4, 'questions', 42, 6),
(5, 'questions', 43, 6),
(6, 'questions', 44, 6),
(71, 'announcements', 72, 6),
(73, 'announcements', 73, 6),
(27, 'questions', 1, 7),
(26, 'questions', 56, 7),
(30, 'blogs', 58, 7),
(31, 'blogs', 59, 7),
...
(34, 'blogs', 59, 8),
(35, 'blogs', 60, 8),
(70, 'announcements', 37, 9),
(76, 'announcements', 66, 12),
(77, 'announcements', 67, 12),
(78, 'announcements', 76, 12);
...
ALTER TABLE `tags_relationships`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `unique_index` (`typeId`,`tagId`,`type`) USING BTREE,
  ADD KEY `tagId` (`tagId`);
...
--
ALTER TABLE `answers`
  ADD CONSTRAINT `answers_ibfk_1` FOREIGN KEY (`owner`) REFERENCES `users` (`id`);
