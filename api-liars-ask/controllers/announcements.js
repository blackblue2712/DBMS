...
module.exports.getAnnouncements = (req, res) => {
    ...
    con.query(query, (err, acms) => {
        if(err) return res.status(400).json( {message: "Error occur"} );
        return res.status(200).json( {message: `${acms[0].length} announcements loaded`, payload: acms[0]});
    })
}
...
module.exports.putEditAcm = (req, res) => {
    ...
    let query = "";
    if(body) {
        query = `CALL updateAcm('${addslashes(title)}', '${addslashes(body)}', ${Boolean(isImportant)}, '${JSON.stringify(tagsnameArray)}', ${Number(acm.id)})`;
    } else {
        query = `CALL updateAcm('${addslashes(title)}', '', ${Boolean(isImportant)}, '${JSON.stringify(tagsnameArray)}', ${Number(acm.id)})`;
    }
